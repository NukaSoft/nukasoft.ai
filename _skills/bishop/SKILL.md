...
## Site Inventory (Last Updated: 2026-05-14 21:52 EDT — auto-generated by medbay)

### Devices

| Name | Type | Model | MAC | IP | Firmware | State |
|------|------|-------|-----|-----|----------|-------|
| **US 24 PoE 250W** | `usw` | US24P250 | `xx:xx:xx:xx:xx:bd` | 192.168.1.69 | 7.4.1.16850 | CONNECTED |
| **Nano HD -Izzy** | `uap` | U7NHD | `d0:21:f9:62:4c:ed` | 192.168.1.186 | 6.7.41.15623 | CONNECTED |
| **Nano HD** | `uap` | U7NHD | `78:45:58:e1:48:8b` | 192.168.1.169 | 6.7.41.15623 | CONNECTED |
| **USW Ultra - Izzy’s Room** | `usw` | USM8P | `58:d6:1f:5e:89:7e` | 192.168.0.200 | 2.1.8.971 | CONNECTED |
| **MyNetworkn AP 1** | `uap` | UAPA693 | `xx:xx:xx:xx:xx:45` | 192.168.1.148 | 8.5.21.18681 | CONNECTED |
| **Nuka-Soft** | `udm` | UDMPRO | `xx:xx:xx:xx:xx:01` | 192.168.1.1 (LAN) / [PUBLIC-IP] (WAN) | 5.0.16.30692 | CONNECTED |

### Networks

| Network | Purpose | VLAN | Subnet | DHCP | DHCP Range |
|---------|---------|------|--------|------|------------|
| Default | corporate | Untagged | 192.168.1.1/24 | ✅ | 192.168.0.6-192.168.0.254 |
| MyVPN | remote-user-vpn | — | 192.168.3.1/24 | — | — |
| NukaSoft | corporate | 10 | 192.168.10.1/24 | ✅ | 192.168.10.100-192.168.10.254 |
| IoT | corporate | 20 | 192.168.20.1/24 | ✅ | 192.168.20.100-192.168.20.254 |
| Guest | corporate | 30 | 192.168.30.1/24 | ✅ | 192.168.30.100-192.168.30.254 |
| EV | corporate | 40 | 192.168.40.1/24 | ✅ | 192.168.40.100-192.168.40.254 |

### Wireless

| SSID | Security | Status | VLAN |
|------|----------|--------|------|
| MyNetwork | wpapsk | ✅ Enabled | Guest (30) |
| MyNetwork-NukaSoft | wpapsk | ✅ Enabled | NukaSoft (10) |
| MyNetwork-IoT | wpapsk | ✅ Enabled | IoT (20) |
| MyNetwork-EV | wpapsk | ✅ Enabled | EV (40) |
...
[US-24-250W PoE Switch] 192.168.1.69
    ...
    |— [My-Server] 192.168.1.200 (Ubuntu server — Claude Code, Nginx, Dashboard)
    |       ├── Nginx :80 → dashboard.example.ai
...
### Services on My-Server (192.168.1.200)

| Service | Port | URL | Systemd Unit | Status |
|---------|------|-----|-------------|--------|
| **Nginx reverse proxy** | 80 | `http://my-server.local/` | `nginx.service` (system) | Active |
| **Skippy Dashboard** | 4820 | `http://dashboard.example.ai/dashboard/` | `skippy-dashboard.service` (user) | Active |
...
|--------|------|-------|---------|
| `dashboard.example.ai` | A | 192.168.1.200 | LAN access to Skippy Dashboard |
...
- **Skippy Dashboard**: `http://dashboard.example.ai/dashboard/` (Nginx → port 4820).
...
| My-Server | My-Server | 192.168.1.200 | `xx:xx:xx:xx:xx:4f` | Worker machine (Claude Code) |
