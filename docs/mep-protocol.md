...
### On Session Start:
1. Run `git pull` to get latest from the other machine
2. Read `machines/handoff.md` — cross-machine state transfer
3. Read `TASKS.md` — current task queue
4. Check for today's journal: `daily/YYYY-MM-DD.md`
5. Brief the user (2-3 lines max)

### On Session End:
1. Update `machines/handoff.md` with: what happened, what's pending, what the other machine needs to know
2. Commit and push
```
...
**What happened:** Built Piper polling infra, fixed Mac git auth, invented MEP
**What's pending:** Run poller on Hot Rod, schedule recurring poll
**Watch out for:** gh CLI not authed on Mac, use Hot Rod for GitHub API calls
```
...
The protocol requires no human action to function. The identity file loads automatically. The instructions are mandatory ("MANDATORY" in the heading). The agent reads, follows, and executes without prompting.
...
2. Add a `CLAUDE.md` with the Session Protocol section above
3. Add a `machines/handoff.md` file
4. Clone on both machines
5. That's it

### Multi-Account SSH (if needed)

If your machines use different GitHub accounts, configure SSH host aliases:
...
Remote URL: `git@github-personal:you/repo.git`
...
*[github.com/NukaSoft/nukasoft.ai](https://github.com/NukaSoft/nukasoft.ai)*
